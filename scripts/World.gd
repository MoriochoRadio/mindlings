...
## 계측 결과: 학습률이 높으면 보상-가소성이 세대를 거쳐 가중치를 클램프(±8)까지 밀어붙여 일부 혈통의
## 채집 행동이 망가지고 → 기아 붕괴/전멸로 이어졌다(학습 OFF면 인구 안정). 그래서 학습은 '천천히 다듬는'
## 수준으로 약하게 둔다(아래 learning_rate). 행동 적응은 유지하되 폭주는 막는다.
## 기본 학습률(작게 — 안정). 개체별 실제 학습률 = 이 값 × 유전 가소성.
## 안정성은 learning_rate를 죽여서가 아니라 weight_homeostasis(아래)로 잡는다 → 학습은 '의미 있게' 유지.
@export var learning_rate: float = 0.02
## 가중치 항상성(생애 학습 안정화 — 근본). 매 틱 가중치를 유전 기준선으로 이만큼 비율로 끌어당긴다.
## 계측: 학습이 세대/생애에 걸쳐 가중치를 클램프(±8)까지 포화시켜 채집이 망가지고 전멸로 이어졌다.
## 이 탄력 복원으로 '오래 살아도 가중치가 표류·포화하지 않게' 막는다. 0이면 OFF(옛 불안정 동작).
## 클수록 학습이 빨리 기준선으로 풀려 더 안정(대신 학습 지속성↓). 0.01≈시정수 100틱(~1.7s).
@export_range(0.0, 0.1) var weight_homeostasis: float = 0.01
...
# 관계: 이미 '친구가 되었어요' 토스트를 띄운 쌍(양쪽 개체가 동시에 보고해도 1회만). 키="작은id_큰id".
var _friend_pairs: Dictionary = {}
...
func reproduce(parent: Creature) -> bool:
	...
	parent.children_count += 1  # 인생 기록: 부모의 자녀 수
...
## 개체가 죽을 때 호출(평균 수명 통계 + 최근 수명 롤링 + 상실 연출). 개체 참조로 위치·즐겨찾기를 읽는다.
func report_death(c: Creature) -> void:
	_death_age_sum += c.age
	_death_count += 1
	_recent_ages.append(c.age)
	if _recent_ages.size() > _RECENT_MAX:
		_recent_ages.pop_front()
	# 죽음의 순간(삶의 이벤트): 조용히 흩어지는 옅은 이펙트.
	spawn_life_effect(c.position, 20.0, Color(0.62, 0.66, 0.76))
	# 즐겨찾기(아끼던) 개체는 특별히 작별을 알린다(애착 — 상실이 의미를 갖게). 그 외엔 조용한 이펙트만.
	if c.favorited:
		_toast("🕊️ 아끼던 %s가 세상을 떠났어요. (%d세대 · %.0f살 · 자녀 %d)" % [
			c.nickname, c.generation, c.age, c.children_count])
...
## from = 발신자(개체) — 듣는 쪽이 '친구의 경보'를 더 크게 받게 하는 데 쓴다(관계가 소통에 반영).
func emit_alarm(pos: Vector2, strength: float, from: Object = null) -> void:
	if _alarms.size() >= max_alarms:
		return
	_alarms.append({"pos": pos, "strength": clampf(strength, 0.0, 1.0), "from": from})
...
## listener를 넘기면 '친구의 경보'를 유대만큼 더 크게 듣는다(친구 경보 우선 — 관계가 행동에 반영).
func hear_alarm(pos: Vector2, listener: Object = null) -> Dictionary:
	...
	for a in _alarms:
		...
		# 친구 가중: 발신자가 듣는 이의 친구면 유대만큼 강하게 듣는다(친구의 위험에 더 크게 반응).
		var from: Object = a.get("from")
		if listener != null and from != null and is_instance_valid(from) and from != listener:
			inten *= 1.0 + listener.get_bond(from) * listener.friend_alarm_bonus
		if inten > best_int:
			best_int = inten
...
## 두 개체의 유대가 친구 임계를 넘으면 호출(Creature). 쌍 단위로 1회만 토스트(양쪽이 보고해도 중복 제거).
func report_friendship(a: Object, b: Object) -> void:
	if a == null or b == null:
		return
	var key: String = _pair_key(a, b)
	if _friend_pairs.has(key):
		return
	_friend_pairs[key] = true
	_toast("💛 %s와 %s가 친구가 되었어요!" % [a.nickname, b.nickname])

## 유대가 임계 아래로 식으면 호출(Creature) — 쌍을 풀어, 다시 친해지면 또 알릴 수 있게(재무장).
func report_unfriend(a: Object, b: Object) -> void:
	if a == null or b == null:
		return
	_friend_pairs.erase(_pair_key(a, b))

## 순서 무관 쌍 키(작은 인스턴스 id_큰 id) — 양쪽 개체가 같은 쌍으로 보고하게.
