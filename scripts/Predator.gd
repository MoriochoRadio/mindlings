...
var _face: float = 1.0  # 바라보는 방향(좌우 뒤집기) — 짐승은 옆모습, 몸을 회전시키지 않는다
...
func _ready() -> void:
	...
	if _body != null:
		_body.visible = false  # 옛 화살촉 폴리곤을 숨기고 _draw로 맹수를 그린다
...
func _physics_process(delta: float) -> void:
	...
	if moving.length() > 0.01:
		...
		if step.length() > 0.001:
			_heading = step.angle()
			if absf(step.x) > 0.02:
				_face = -1.0 if step.x < 0.0 else 1.0
...
## 배고프면 어둑하게, 막 사냥했으면 또렷하게(상태를 색으로). 매 틱 다시 그린다(포식자는 소수).
func _update_color() -> void:
	queue_redraw()

## 위협적인 맹수(옆모습): 어두운 몸통·네 다리·뾰족 귀·붉은 눈·꼬리. '작은 사람들'과 대비되는 포식자.
## 몸은 눕지 않고(회전 X), 바라보는 방향은 좌우 뒤집기(_face). 배고프면 어둑, 사냥 직후 또렷.
func _draw() -> void:
	var t: float = clampf(energy / max_energy, 0.0, 1.0)
	var fur: Color = Color(0.28, 0.12, 0.14).lerp(Color(0.52, 0.18, 0.20), t)
	var dark: Color = fur.darkened(0.4)
	var outline := Color(0.03, 0.02, 0.04, 0.6)

	# 그림자(발밑, 눌린 타원)
	draw_set_transform(Vector2(0.0, 6.0), 0.0, Vector2(1.3, 0.4))
	draw_circle(Vector2.ZERO, 8.0, Color(0.0, 0.0, 0.0, 0.25))
	# 바라보는 방향으로 뒤집기(+x가 앞/머리)
	draw_set_transform(Vector2.ZERO, 0.0, Vector2(_face, 1.0))

	# 다리 네 개(짧고 굵게)
	draw_line(Vector2(-6.0, 2.0), Vector2(-6.5, 7.5), dark, 2.6)
	draw_line(Vector2(-3.0, 3.0), Vector2(-3.2, 8.0), dark, 2.6)
	draw_line(Vector2(3.0, 3.0), Vector2(3.2, 8.0), dark, 2.6)
	draw_line(Vector2(6.0, 2.0), Vector2(6.7, 7.5), dark, 2.6)
	# 꼬리(뒤쪽, -x)
	draw_line(Vector2(-7.5, -1.0), Vector2(-12.0, -4.5), fur, 2.4)
	# 몸통(가로 캡슐)
	draw_circle(Vector2(-5.0, 0.0), 5.2, fur)
	draw_circle(Vector2(5.0, 0.0), 5.0, fur)
	draw_rect(Rect2(-5.0, -5.2, 10.0, 10.4), fur)
	# 머리(앞쪽, +x)
	draw_circle(Vector2(8.5, -1.5), 4.6, fur)
	# 뾰족 귀 두 개
	draw_colored_polygon(PackedVector2Array([Vector2(6.5, -5.5), Vector2(8.0, -9.0), Vector2(9.2, -5.2)]), fur)
	draw_colored_polygon(PackedVector2Array([Vector2(9.5, -5.6), Vector2(11.2, -8.6), Vector2(11.8, -4.8)]), fur)
	# 주둥이
	draw_circle(Vector2(12.2, -0.5), 2.3, dark)
	# 붉은 눈(위협)
	draw_circle(Vector2(9.5, -2.2), 1.3, Color(1.0, 0.75, 0.2))
	draw_circle(Vector2(9.7, -2.2), 0.6, Color(0.7, 0.1, 0.05))
	draw_set_transform(Vector2.ZERO, 0.0, Vector2.ONE)  # 변환 리셋
